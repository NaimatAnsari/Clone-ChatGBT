    ...
    <div className='h-screen w-screen bg-[#050509] flex'>
      <div className='w-[20%] px-6 h-screen bg-[#0c0c15] text-white p-4'>
        <div className='flex items-center gap-2 justify-start'>
          <img src="../public/assets/images/logo.png" className='h-10 rounded-full  ' alt="Logo" />
          <h1 className='text-[30px] text-center py-6  font-bold'>ChatGBT</h1>
        </div>

        <div className='flex items-center justify-start'>
          <button className='w-full h-[50px] text-white font-[600] text-[18px] outline-none rounded bg-[#4f4fe2] hover:bg-slate-600'>+ New Chat</button>
        </div>
        ...
        {chats.length > 0 ? (
          <div className='h-[80%] overflow-scroll hide-scroll-bar pt-8'>
            {chats.map((item, index) => (
              <div key={index} className={`w-[60%] mx-auto p-4 flex text-white ${item.role === 'assistant' && 'bg-slate-900'} rounded`}>
                <span className='text-white'>
                  {item.role === 'user' ? <i className='bx bx-user-circle p-2 text-[22px]' ></i> : <img src="../public/assets/images/logo.png" className='h-[38px] p-2 rounded-full'/> }
                </span>
                <div className='w-[90%] mt-[7px]'>{item.message}</div>
              </div>
            ))}
          </div>
        ) : (
          <div className='h-[80%] flex flex-col justify-center items-center text-white'>
            ...
            <div className='flex flex-wrap justify-evenly py-4 max-w-[900px]'>
              {example.map((item, index) => (
                <div key={index} className='text-lg text-[13px] mt-3 p-2 font-light min-w-[400px] border rounded cursor-pointer hover:bg-slate-600'>
                  {item}
                </div>
              ))}
            </div>
